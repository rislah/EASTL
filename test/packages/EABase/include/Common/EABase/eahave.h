...
#if (defined(EA_PLATFORM_UNIX) && !(defined(CS_UNDEFINED_STRING) && defined(EA_PLATFORM_CONSOLE))) || \
    defined(__APPLE__) || defined(EA_PLATFORM_ANDROID)
...
#define EA_HAVE_PTHREAD_H \
	1 /* It can be had under Microsoft/Windows with the http://sourceware.org/pthreads-win32/ library */
#else
...
#define EA_HAVE_CPP11_CONTAINERS 1
    ...
    (EA_COMPILER_VERSION >= 4004) // Actually GCC 4.3 supports array and unordered_
#define EA_HAVE_CPP11_CONTAINERS 1
...
#if defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && (_CPPLIB_VER >= 540) && \
    (defined(_HAS_EXCEPTIONS) && _HAS_EXCEPTIONS) // Dinkumware. VS2012+
...
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(EA_HAVE_LIBSTDCPP_LIBRARY) && defined(EA_COMPILER_GNUC) && \
    (EA_COMPILER_VERSION >= 4007) // chrono was broken in glibc prior to 4.7.
#define EA_HAVE_CPP11_CHRONO 1
#elif defined(EA_HAVE_LIBCPP_LIBRARY) && (_LIBCPP_VERSION >= 1)
#define EA_HAVE_CPP11_CHRONO 1
#else
...
#if defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && (_CPPLIB_VER >= 520) && \
    !(defined(_HAS_CPP0X) || _HAS_CPP0X) // Dinkumware. VS2010+
#define EA_HAVE_CPP11_SYSTEM_ERROR 1
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(EA_HAVE_LIBSTDCPP_LIBRARY) && defined(EA_COMPILER_CLANG) && \
    (EA_COMPILER_VERSION >= 301) && !defined(EA_PLATFORM_APPLE)
#define EA_HAVE_CPP11_SYSTEM_ERROR 1
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(EA_HAVE_LIBSTDCPP_LIBRARY) && defined(EA_COMPILER_GNUC) && \
    (EA_COMPILER_VERSION >= 4004) && !defined(EA_PLATFORM_APPLE)
#define EA_HAVE_CPP11_SYSTEM_ERROR 1
#elif defined(EA_HAVE_LIBCPP_LIBRARY) && (_LIBCPP_VERSION >= 1)
#define EA_HAVE_CPP11_SYSTEM_ERROR 1
#else
...
#if defined(EA_PLATFORM_POSIX) /* Posix means Linux, Unix, and Macintosh OSX, among others (including Linux-based \
                                  mobile platforms). */
...
#define EA_HAVE_ISNAN(x)                                                                                            \
	__builtin_isnan(x) /* There are a number of standard libraries for Android and it's hard to tell them apart, so \
	                      just go with builtins */
...
#elif defined(EA_PLATFORM_MINGW) /* Meant to workaround a GCC limitation where cmath undefines isnan, both GCC and \
                                    clang have std::isnan */
#define EA_HAVE_ISNAN(x) std::isnan(x) /* declared in <cmath> */
#define EA_HAVE_ISINF(x) std::isinf(x)
...
#if (defined(EA_PLATFORM_UNIX) && !defined(CS_UNDEFINED_STRING)) || defined(EA_PLATFORM_IPHONE) || \
    defined(EA_PLATFORM_OSX) || defined(CS_UNDEFINED_STRING)
...
#define EA_HAVE_WCHAR_IMPL \
	1 /* Specifies if wchar_t string functions are provided, such as wcslen, wprintf, etc. Implies EA_HAVE_WCHAR_H */
#else
...
#if (defined(EA_PLATFORM_DESKTOP) || defined(EA_PLATFORM_UNIX)) && !defined(EA_PLATFORM_WINRT)
#define EA_HAVE_putenv_IMPL \
	1 /* With Microsoft compilers you may need to use _putenv, as they have deprecated putenv. */
#else
...
#define EA_HAVE_inet_ntop_IMPL \
	1 /* This doesn't identify if the platform SDK has some alternative function that does the same thing; */
#define EA_HAVE_inet_pton_IMPL                                                                                  \
	1 /* it identifies strictly the <arpa/inet.h> inet_ntop and inet_pton functions. For example, Microsoft has \
	     InetNtop in <Ws2tcpip.h> */
#else
...
#if defined(EA_PLATFORM_LINUX) || defined(__CYGWIN__) || (defined(_POSIX_TIMERS) && (_POSIX_TIMERS > 0)) || \
    (defined(EA_PLATFORM_POSIX) && defined(_YVALS) /*Dinkumware*/)
...
#define EA_NO_HAVE_clock_gettime_IMPL 1
    ...
    !defined(EA_PLATFORM_WINRT)
#define EA_HAVE_getcwd_IMPL                                                                                         \
	1 /* With Microsoft compilers you may need to use _getcwd, as they have deprecated getcwd. And in any case it's \
	     present at <direct.h> */
#else
...
// This EA_HAVE is deprecated, as EA_COMPILER_NO_NULLPTR is more appropriate, given that nullptr is a compiler-level
...
#define EA_HAVE_std_terminate_IMPL \
	1 /* iOS doesn't appear to provide an implementation for std::terminate under the armv6 target. */
#else
...
// <functional>: std::function, std::mem_fn, std::bad_function_call, std::is_bind_expression, std::is_placeholder,
// std::reference_wrapper, std::hash, std::bind, std::ref, std::cref.
